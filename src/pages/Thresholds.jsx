...
export default function Thresholds() {
  // State untuk melacak lantai yang sedang dipilih (1-4)
  ...
  // Efek memicu reload data setiap kali tab 'selectedFloor' berubah
  useEffect(() => {
    const token = localStorage.getItem("token");
    axios.get(`http://192.168.1.105:5000/api/thresholds/${selectedFloor}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
      .then(res => setForm(res.data))
      .catch(err => console.error("Thresholds Fetch Error:", err));
  }, [selectedFloor]);
  ...
  const handleSave = async (e) => {
    ...
    const token = localStorage.getItem("token");
    try {
      // Mengirimkan data form beserta info target floorLevel-nya
      await axios.post('http://192.168.1.105:5000/api/thresholds', {
        ...form,
        floorLevel: selectedFloor
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert(`Konfigurasi ambang batas Lantai ${selectedFloor} berhasil diperbarui dan disinkronkan ke DB & Broker MQTT!`);
    } catch (err) {
      console.error("Save Thresholds Error:", err);
      alert(`Gagal menyimpan ambang batas Lantai ${selectedFloor}`);
    }
  };

  const handleReset = () => {
    if (window.confirm(`Kembalikan pengaturan Lantai ${selectedFloor} ke default pabrik?`)) {
      setForm({
        floorLevel: selectedFloor,
        tempMin: 25.0,
        tempMax: 32.0,
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl flex items-start gap-4">
        ...
        <div>
          <h4 className="text-sm font-bold text-emerald-900">Panduan Konfigurasi Multi-Lantai</h4>
          <p className="text-xs text-emerald-700 leading-relaxed">
            Pilih nomor lantai terlebih dahulu sebelum mengubah parameter. Parameter di bawah ini akan menentukan kapan aktuator pada lantai tersebut menyala secara otomatis.
          </p>
        </div>
      </div>

      {/* NAVIGASI SELEKTOR LANTAI (1 - 4) */}
      <div className="bg-slate-100 p-1.5 rounded-2xl inline-flex w-full md:w-auto gap-1 border border-slate-200">
        {[1, 2, 3, 4].map((floor) => (
          <button
            ...
            className={`flex-1 md:flex-initial px-6 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition flex items-center justify-center gap-2 ${selectedFloor === floor
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  step="0.1"
                  value={form.tempMin || ''}
                  onChange={e => setForm({ ...form, tempMin: parseFloat(e.target.value) || 0 })}
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  step="0.1"
                  value={form.tempMax || ''}
                  onChange={e => setForm({ ...form, tempMax: parseFloat(e.target.value) || 0 })}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none font-semibold text-sm"
                />
                <p className="text-[10px] text-slate-400 italic">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  step="0.1"
                  value={form.mediaMin || ''}
                  onChange={e => setForm({ ...form, mediaMin: parseFloat(e.target.value) || 0 })}
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  step="0.1"
                  value={form.mediaMax || ''}
                  onChange={e => setForm({ ...form, mediaMax: parseFloat(e.target.value) || 0 })}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-semibold text-sm"
                />
                <p className="text-[10px] text-slate-400 italic">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={e => setForm({ ...form, humAirMin: parseFloat(e.target.value) || 0 })}
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={e => setForm({ ...form, humAirMax: parseFloat(e.target.value) || 0 })}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-semibold text-sm"
                />
                <p className="text-[10px] text-slate-400 italic">
